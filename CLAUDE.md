...
- **Phase 1 (done)** — Cargo workspace scaffold (`apps/shell`, `xtask`, `crates/*`). `crates/dom` has a real arena-based DOM tree (generation-tagged `NodeId`).
- **Phase 2 (done)** — `quickjs-sys` vendors QuickJS-ng as a git submodule and compiles it via a `cc` build script; `js-runtime` wraps it in a safe `Runtime`/`Context`. Real DOM bindings (`Node` class, `document.getElementById`, `addEventListener`/`dispatchEvent`), `setTimeout`/`setInterval`/`requestAnimationFrame`, `performance.now()`, `crypto.getRandomValues`. No real event loop yet (belongs with `profile`'s per-tab process) — `Context::run_pending_timers()` is a manual/cooperative pump the host must call explicitly.
- **Phase 3 (done)** — `css` (tokenizer, selector/declaration parser, cascade — descendant combinator only, no `>`/`+`/`~`/pseudo-classes/attribute selectors). `layout-engine` (block + single-line flex formatting, inherited `font-size`/`color`, real inline formatting via `cosmic-text` text shaping/line-wrapping, `background-color`). `render` does real GPU rasterization (`wgpu`, one draw call/solid-color quads, no borders/images/shadows), `Canvas2D` (`fillRect`/`clearRect`/`getImageData`, solid rects only), and glyph compositing. `webgl` implements `create_shader`/`create_program`/`draw_triangles` over `wgpu` with real GLSL ES 3.00 shader compilation via naga (no ES 1.00/WebGL1).
- **Phase 4 (in progress)** — the bulk of the engine:
  - `ipc`: lock-free double-buffered shared-memory frame transport.
  - `profile`/`profile-worker`: spawns a sandboxed child process per tab running a real ~60Hz vsync loop (dom+css+js-runtime+layout-engine+render), with `NAVIGATE`/`RELOAD`/`PING`/`QUIT`/`CLICK`/`FILL` over stdin. Fetches real HTML/CSS (`<link>`, `@import`, `@media`), sends/stores cookies, and threads per-profile proxy/DNS/GPU-adapter choice through every network call.
  ...
  - `js-runtime` Fetch API: `fetchSync`/`fetch`/`XMLHttpRequest`, backed by real `Headers`/`Request`/`Response` classes (queryable headers, `text()`/`json()`/`arrayBuffer()`, cloneable) — not a plain `{ok, status, body}` stand-in. Also: `Blob`/`File`/`URL.createObjectURL`, `Notification` (permission state machine, no real OS toast), `navigator.clipboard`, `OfflineAudioContext` (real sine synthesis/gain/mixing, no non-sine waveforms), `document.cookie`, `indexedDB.open` with structured-clone value bridging.
