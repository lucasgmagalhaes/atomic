...
- **Phase 3 (started)**: `css` has a real tokenizer (`lexer.rs`, a practical CSS Syntax Module Level 3 subset — no `url()`, no unicode-range), a selector/declaration parser (`parser.rs`, descendant combinator only, no `>`/`+`/`~`/pseudo-classes/attribute selectors, declaration values kept as raw tokens), and selector matching + cascade ordering (`cascade.rs`, `selector_matches`/`matching_declarations` — takes a plain ancestor-chain snapshot, doesn't depend on `dom`). No computed-style resolution (merging cascaded declarations into typed values) and no `layout-engine`/`render`/`webgl` work yet — those are next.
