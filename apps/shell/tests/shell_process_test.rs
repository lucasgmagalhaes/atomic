//! Process-level smoke test for the `shell` binary itself — regression
//! coverage for a real crash observed while manually verifying the
//! automation panel: a first `shell.exe` launch died within a couple of
...
//! assert the one thing that matters: launched fresh, `shell.exe` must
