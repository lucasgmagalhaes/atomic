...
1. `[x]` `crates/cef`: real coordinate-based input injection (`send_mouse_click_event`/`send_mouse_move_event`/`send_mouse_wheel_event`/`send_key_event`) on `cef_profile_worker`'s existing protocol (2026-09-16) — needed by both content panes and the chrome browser, built and proven against the already-proven worker first.
2. `[x]` **No new binary needed** (2026-09-16 finding, `crates/cef/tests/chrome_bridge_test.rs`, 2/2 passing) — `cef_profile_worker` already supports every primitive the chrome browser needs: `NAVIGATE` to a local `data:`/`file://` bundle instead of a profile's own URL, `EVAL` for Rust→JS state push, `CONSOLE`/`CLICK_AT` for the JS→Rust action channel. A "chrome browser" is just `profile::Profile::spawn` pointed at the local bundle under its own `shmem_name` — proven end to end with a minimal hand-written page (not yet the real mockup): a real `EVAL` call updates a real DOM node, and a real coordinate click on a real button's `onclick` reaches Rust through the exact same `CONSOLE`-draining code path already built for page console messages. The step originally planned here (a separate `cef_chrome` binary/mode) turned out to be unnecessary — don't build one.
...
4. `[ ]` `crates/atomic`: spawn a chrome `Profile` at app startup (own `shmem_name`, e.g. `"atomic-chrome"`, navigated to the ported bundle instead of a profile URL), replace `egui`-drawn chrome panels with the chrome texture + native compositing described above, wire the bridge to real `AtomicApp` methods one screen at a time (toolbar first, since every other screen depends on tab/workspace switching already working).

Do not attempt to port the whole mockup in one pass — land the toolbar end-to-end (real click reaching a real `switch_workspace` call, real state pushed back and reflected in the page) as the first working slice, matching how `cef_smoke`/`cef_profile_worker` were each proven standalone before the next piece was built on top.
