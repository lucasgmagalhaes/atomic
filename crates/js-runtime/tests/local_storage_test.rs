...
fn without_storage_getitem_returns_null_and_length_is_zero() {
  ...
  assert_eq!(
    ctx
      .eval("localStorage.getItem('k') === null", "<test>")
      .unwrap(),
    "true"
  );
...
fn set_then_get_a_real_item() {
  ...
  ctx
    .eval("localStorage.setItem('theme', 'dark');", "<test>")
    .unwrap();
  assert_eq!(
...
fn remove_item_deletes_the_key() {
  ...
  ctx
    .eval("localStorage.setItem('a', '1');", "<test>")
    .unwrap();
  ctx.eval("localStorage.removeItem('a');", "<test>").unwrap();
  assert_eq!(
    ctx
      .eval("localStorage.getItem('a') === null", "<test>")
      .unwrap(),
    "true"
  );
  assert_eq!(ctx.eval("localStorage.length", "<test>").unwrap(), "0");
...
fn key_returns_a_key_by_index() {
  ...
  ctx
    .eval("localStorage.setItem('only', 'value');", "<test>")
    .unwrap();
...
fn local_and_session_storage_are_independent() {
  let rt = Runtime::new();
  let ctx =
    Context::with_storage(&rt, dom::Dom::new(), "example.com", temp_dir("independent")).unwrap();
...
fn persists_across_separate_contexts_sharing_a_storage_dir() {
  ...
  let dir = temp_dir("persist");
  {
    ...
    ctx
      .eval("localStorage.setItem('k', 'persisted');", "<test>")
      .unwrap();
  }
