...
fn spawn_busy_process() -> std::process::Child {
    ...
    std::process::Command::new("powershell")
        .args([
            ...
            "-NonInteractive",
            "-Command",
            "$sw = [Diagnostics.Stopwatch]::StartNew(); while ($sw.Elapsed.TotalSeconds -lt 2) { }",
        ])
...
fn sample_reports_real_nonzero_memory_for_a_running_process() {
    ...
    assert!(
        ...
        "a real process should have nonzero working-set memory"
    );
...
fn cpu_time_increases_over_a_real_busy_loop() {
    ...
    assert!(
        ...
        "cpu_time is cumulative, must never decrease"
    );
    assert!(
        after.cpu_time > before.cpu_time,
        "a real busy loop should burn measurable CPU time between samples"
    );
...
fn cpu_percent_of_a_process_saturating_one_core_is_about_100_over_core_count() {
    ...
    assert!(
        (pct - 25.0).abs() < 0.01,
        "expected ~25% (100% / 4 cores), got {pct}"
    );
}
