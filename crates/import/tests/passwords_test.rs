...
fn imports_and_decrypts_a_real_login_row() {
    ...
    conn.execute("CREATE TABLE logins (origin_url TEXT, username_value TEXT, password_value BLOB)", []).unwrap();
    ...
    assert_eq!(passwords[0].password, "hunter2-but-real", "real AES-256-GCM decryption should recover the exact plaintext");
...
fn a_row_with_a_v20_prefix_is_skipped_as_an_unsupported_scheme() {
    ...
    let passwords = import_passwords(&path, &[0u8; 32]).expect("import itself should still succeed");
    assert!(passwords.is_empty(), "a real v20 (App-Bound Encryption) row should be skipped, not misread as v10/v11");
