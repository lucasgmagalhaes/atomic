...
use render::{composite_images, ImageQuad};
...
fn solid_image(width: u32, height: u32, color: [u8; 4]) -> Rc<DecodedImage> {
    ...
    Rc::new(DecodedImage { width, height, rgba })
}
...
fn composites_a_real_image_at_1to1_scale() {
    ...
    let quad = ImageQuad { x: 0.0, y: 0.0, width: 4.0, height: 4.0, image };
...
fn composites_at_an_offset_and_leaves_the_rest_untouched() {
    ...
    let quad = ImageQuad { x: 3.0, y: 3.0, width: 2.0, height: 2.0, image };
...
fn nearest_neighbor_upscaling_samples_real_source_pixels() {
    ...
    let quad = ImageQuad { x: 0.0, y: 0.0, width: 4.0, height: 1.0, image };
...
fn transparent_source_pixels_do_not_overwrite_the_destination() {
    ...
    let quad = ImageQuad { x: 0.0, y: 0.0, width: 2.0, height: 2.0, image };
...
fn a_zero_sized_quad_is_skipped_without_panicking() {
    ...
    let quad = ImageQuad { x: 0.0, y: 0.0, width: 0.0, height: 0.0, image };
