...
2. ~~`dataset`~~ — done, read side: `element.dataset` (`crates/js-runtime/src/dom_bindings.rs`, `sync_dataset`/`node_dataset_get`) reflects every live `data-*` attribute under its camelCase key (`data-user-id` -> `dataset.userId`), stable per-node object identity (same `CLASS_LIST_OBJECTS`-style cache, resynced on every access and after `setAttribute`/`removeAttribute`). Deliberately not wired for writes (`el.dataset.foo = "x"` sets a plain JS property, doesn't touch a `data-foo` attribute) — real per-key reflection needs a `Proxy`, which this crate's `quickjs-sys` bindings don't expose (documented, same scope-down `localStorage`'s missing bracket access already has). `attributes`/`NamedNodeMap` still open; then element-specific properties for form controls and anchors.
