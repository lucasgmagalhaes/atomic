...
fn write_text_then_read_text_round_trips_through_the_real_os_clipboard() {
    ...
    let done = pump_until(&ctx, |c| c.eval("seen !== null", "<test>").unwrap() == "true", Duration::from_secs(5));
    assert!(done, "clipboard round trip should complete quickly against a real OS clipboard");
    assert_eq!(ctx.eval("seen", "<test>").unwrap(), "nimble-clipboard-test-value");
}
...
fn navigator_clipboard_is_present_on_a_plain_context() {
    let rt = Runtime::new();
    let ctx = Context::new(&rt);
    assert_eq!(ctx.eval("typeof navigator.clipboard.writeText", "<test>").unwrap(), "function");
    assert_eq!(ctx.eval("typeof navigator.clipboard.readText", "<test>").unwrap(), "function");
}
