...
fn put_and_get_round_trip() {
    ...
    store.put("p1", "{\"name\":\"alice\"}").unwrap();
    assert_eq!(store.get("p1"), Some("{\"name\":\"alice\"}"));
...
fn multiple_stores_are_independent() {
    ...
    assert_eq!(db.store("a").unwrap().get("k"), Some("from-a"));
    assert_eq!(db.store("b").unwrap().get("k"), Some("from-b"));
}
...
fn persists_stores_and_data_across_separate_open_calls() {
    ...
    assert_eq!(reopened.store("profiles").unwrap().get("p1"), Some("alice"));
    assert_eq!(reopened.store("settings").unwrap().get("theme"), Some("dark"));
}
...
fn index_finds_keys_by_a_secondary_value() {
    ...
    let mut eng: Vec<&str> = store.get_by_index("by_team", "eng").into_iter().map(|(_, v)| v).collect();
    eng.sort();
    assert_eq!(eng, ["alice", "bob"]);

    let design = store.get_by_index("by_team", "design");
    assert_eq!(design, vec![("u3", "carol")]);
