...
// `El`/`Button`/`Row`/`List`/`mount` kit (`lib/ui.js`). The submit button's
// click is dispatched by `ChromeEngine::handle_click` same as everything
// else, but the URL text itself never passes through JS at all — the host
// reads the input's live `.value` directly off the DOM (see
