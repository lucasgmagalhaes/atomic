...
fn without_storage_getitem_returns_null_and_length_is_zero() {
    ...
    assert_eq!(
        ctx.eval("localStorage.getItem('k') === null", "<test>")
            .unwrap(),
        "true"
    );
    ...
    assert!(ctx
        .eval("localStorage.setItem('k', 'v'); 'ok'", "<test>")
        .is_ok());
}
...
fn set_then_get_a_real_item() {
    ...
    ctx.eval("localStorage.setItem('theme', 'dark');", "<test>")
        .unwrap();
    assert_eq!(
        ctx.eval("localStorage.getItem('theme')", "<test>").unwrap(),
        "dark"
    );
...
fn remove_item_deletes_the_key() {
    let rt = Runtime::new();
    let ctx =
        Context::with_storage(&rt, dom::Dom::new(), "example.com", temp_dir("remove")).unwrap();

    ctx.eval("localStorage.setItem('a', '1');", "<test>")
        .unwrap();
    ctx.eval("localStorage.removeItem('a');", "<test>").unwrap();
    assert_eq!(
        ctx.eval("localStorage.getItem('a') === null", "<test>")
            .unwrap(),
        "true"
    );
    assert_eq!(ctx.eval("localStorage.length", "<test>").unwrap(), "0");
...
fn key_returns_a_key_by_index() {
    ...
    ctx.eval("localStorage.setItem('only', 'value');", "<test>")
        .unwrap();
    assert_eq!(ctx.eval("localStorage.key(0)", "<test>").unwrap(), "only");
    assert_eq!(
        ctx.eval("localStorage.key(5) === null", "<test>").unwrap(),
        "true"
    );
}
...
fn local_and_session_storage_are_independent() {
    let rt = Runtime::new();
    let ctx = Context::with_storage(&rt, dom::Dom::new(), "example.com", temp_dir("independent"))
        .unwrap();

    ctx.eval(
        "localStorage.setItem('k', 'from-local'); sessionStorage.setItem('k', 'from-session');",
        "<test>",
    )
    .unwrap();
    assert_eq!(
        ctx.eval("localStorage.getItem('k')", "<test>").unwrap(),
        "from-local"
    );
    assert_eq!(
        ctx.eval("sessionStorage.getItem('k')", "<test>").unwrap(),
        "from-session"
    );
}
...
fn persists_across_separate_contexts_sharing_a_storage_dir() {
    ...
    let dir = temp_dir("persist");
    {
        ...
        ctx.eval("localStorage.setItem('k', 'persisted');", "<test>")
            .unwrap();
    }
    let ctx2 = Context::with_storage(&rt, dom::Dom::new(), "example.com", &dir).unwrap();
    assert_eq!(
        ctx2.eval("localStorage.getItem('k')", "<test>").unwrap(),
        "persisted"
    );
}
