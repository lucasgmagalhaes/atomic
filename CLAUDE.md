...
- **Phase 2 (in progress)**: `quickjs-sys` vendors QuickJS-ng v0.16.2 as a git submodule (`crates/js-runtime/quickjs-sys/vendor/quickjs-ng`) and compiles it via a `cc` build script; hand-written FFI bindings cover runtime/context lifecycle, `JS_Eval`, and native-function registration (`JS_NewCFunction2` + context opaque data). `js-runtime` wraps it in a safe `Runtime`/`Context` with `eval() -> Result<String, EvalError>`. `dom` gained `find_by_id`/`text_content`/`set_text_content`, exposed to JS as two placeholder globals (`__dom_get_text_by_id`, `__dom_set_text_by_id`) via `Context::with_dom()` — **not** the real `getElementById`/`textContent` API, which needs JS-side object identity for DOM nodes (a node "class" with a `NodeId` handle) that doesn't exist yet. `performance.now()` is bound (per-process time origin — no document/navigation concept exists yet to give it a real per-document origin). Still open from the phase 2 roadmap line: `addEventListener`, timers, `requestAnimationFrame`, Page Visibility, `crypto.getRandomValues` — timers/rAF specifically need an event loop, which isn't designed yet (deferred: it belongs with `profile`/`ipc` in phase 4, where the per-tab process actually runs one).
