...
fn open_with_keychain_uses_a_distinct_file_from_the_plain_vault() {
    ...
    file_vault
        .set("login.email", "file-backed@example.com")
        .expect("set should succeed");

    let mut keychain_vault = vault_ui::open_with_keychain(&dir)
        .expect("real Windows Credential Manager should back this");
    ...
    keychain_vault
        .set("login.email", "keychain-backed@example.com")
        .expect("set should succeed");
    assert_eq!(
        file_vault.get("login.email"),
        Some("file-backed@example.com"),
        "the file-backed vault must be unaffected by the keychain-backed one"
    );

    let reopened = vault_ui::open_with_keychain(&dir)
        .expect("reopen should reuse the real keychain-stored key");
    assert_eq!(
        reopened.get("login.email"),
        Some("keychain-backed@example.com"),
        "a real reopen should see the keychain-backed vault's own persisted entry"
    );
