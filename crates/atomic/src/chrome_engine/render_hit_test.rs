...
use neutron::paint::{build_display_list, build_glyph_list, composite_glyphs, GpuRenderer};
...
impl<'rt> ChromeEngine<'rt> {
    ...
    /// and updates `neutron::dom::Dom`'s hover state to match, skipping the write
    ...
    /// caller that mutates `neutron::dom::Dom` directly via `Context::dom_mut`,
    ...
    pub(crate) fn update_hover(&mut self, width: u32, x: f64, y: f64) {
        ...
        let hit = neutron::layout::hit_test(&tree, x, y);
        let dom = self
            .ctx
            .dom()
    ...
    pub(crate) fn resolve_click_target(&self, width: u32, x: f64, y: f64) -> Option<String> {
        let tree = self.layout(width);
        let node = neutron::layout::hit_test(&tree, x, y)?;
        let dom = self
            .ctx
            .dom()
            .expect("ChromeEngine always builds its context over a dom");
