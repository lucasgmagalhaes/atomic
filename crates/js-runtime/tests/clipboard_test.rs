...
fn write_text_rejects_on_a_missing_argument() {
  ...
  ctx
    .eval(
      "globalThis.err = null; navigator.clipboard.writeText().catch((e) => { err = e.message; });",
      "<test>",
    )
    .unwrap();
...
fn navigator_clipboard_is_present_on_a_plain_context() {
  ...
  assert_eq!(
    ctx
      .eval("typeof navigator.clipboard.writeText", "<test>")
      .unwrap(),
    "function"
  );
  assert_eq!(
    ctx
      .eval("typeof navigator.clipboard.readText", "<test>")
      .unwrap(),
    "function"
  );
}
