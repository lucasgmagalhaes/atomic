...
6. `[ ]` **Resource Loader** (one shared URL→security→CSP→mixed-content→cache→network→decode pipeline). Today each of HTML/CSS/`<link>`/`@import`/cookies has its own near-duplicate fetch path in `profile_worker.rs`.
7. `[ ]` **Structured Clone** (needed before MessageChannel/Worker messaging is spec-correct — see P4). `workers` already does real OS-thread parallelism, but messages are hand-escaped JS string literals, not structured clone.
