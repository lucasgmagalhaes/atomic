...
fn a_left_float_sits_flush_to_the_containing_blocks_left_edge() {
  ...
  let sheet =
    parse_stylesheet("div { width: 200px; } div div { float: left; width: 50px; height: 40px; }");
...
fn a_right_float_sits_flush_to_the_containing_blocks_right_edge() {
  ...
  let sheet =
    parse_stylesheet("div { width: 200px; } div div { float: right; width: 50px; height: 40px; }");
  let mut tree = build_box_tree(&d, container, &sheet).unwrap();
  layout_block(&mut tree, 800.0, 0.0, 0.0);
...
fn a_non_cleared_sibling_after_a_float_can_overlap_it_visually() {
  ...
  let sheet =
    parse_stylesheet("div { width: 200px; } #float { float: left; width: 50px; height: 100px; }");
  let mut tree = build_box_tree(&d, container, &sheet).unwrap();
  layout_block(&mut tree, 800.0, 0.0, 0.0);
...
fn a_container_grows_its_own_height_to_contain_its_floats() {
  ...
  let sheet =
    parse_stylesheet("div { width: 200px; } div div { float: left; width: 50px; height: 100px; }");
  let mut tree = build_box_tree(&d, container, &sheet).unwrap();
  layout_block(&mut tree, 800.0, 0.0, 0.0);
