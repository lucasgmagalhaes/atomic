...
**`FULL_SPEC.md` is the source of truth for granular JS-engine status and architecture rules.** Part A is a checklist (done/needed, with file references) of the JS runtime, DOM classes/properties/functions, events, CSS/layout, paint/compositing, browser Web API classes, and navigation/security/lifecycle — check it (and keep it updated) instead of asking "is X implemented" from scratch or re-deriving it from source. Part B is the architecture/optimization playbook (shared-infrastructure-over-API-specific-code rules, invalidation/caching conventions, the P0–P5 implementation priority order) — read it before adding a new Web API's backing infrastructure, not just before optimizing. The "Implementation status" section below deliberately does not duplicate what Part A already tracks.
...
JS-engine specifics (runtime, DOM, events, CSS/layout, paint, Web API classes) live in `FULL_SPEC.md`, not here. What follows is the surrounding, non-JS-engine infrastructure the matrix doesn't track:
