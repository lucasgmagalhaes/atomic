...
/// Fetches and registers every real `@font-face` (`css::FontFaceRule`)
...
/// handed to `layout_engine::register_font_face` to load into the one
...
pub(crate) fn load_font_faces(
    ...
    for face in &sheet.font_faces {
        ...
        layout_engine::register_font_face(response.body.clone());
    }
}
