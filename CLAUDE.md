...
## Three-scope encapsulation (decided 2026-09-16)

The repo has three real scopes, each its own top-level `crates/*` directory: **`crates/atomic`** (the GUI shell binary — see `crates/atomic/README.md`), **`crates/atomicjs`** (experimental JS compiler/VM, deliberately its own isolated single-crate workspace — see `crates/atomicjs/README.md`), and **`crates/neutron`** (the HTML/CSS/JS compile+layout+paint+execute pipeline, a facade over 11 sub-crates — see `crates/neutron/README.md`). Everything outside `crates/neutron/` depends on the `neutron` facade only, never on its sub-crates (`css`/`html`/`dom`/`atoms`/`layout-engine`/`render`/`webgl`/`image_decode`/`js-runtime`/`workers`) directly — enforced by `cargo run -p xtask -- check-neutron-boundary` (wired into the pre-commit hook). Full reasoning in `spec/proposals/NEUTRON_ENCAPSULATION.md`. `crates/atomic` used to be `apps/shell` (package `shell`) — moved and renamed so the three scopes line up structurally; if you find a leftover `apps/shell`/`shell::`/`js_runtime::`/bare `dom::`/`css::`/etc. reference outside `crates/neutron/`, it's a miss from that move, fix it to `neutron::*` rather than treating it as intentional.
