...
fn evaluate_returns_the_scripts_stringified_completion_value() {
    ...
    let result = profile.evaluate("2 + 3").expect("protocol should not fail against a live worker");
    assert_eq!(result.expect("a valid expression should not throw"), "5", "the completion value should come back stringified");
    ...
    let result = profile.evaluate("document.createElement('div').nodeName").expect("protocol should not fail");
    assert_eq!(
...
fn evaluate_reports_a_throwing_script_and_stays_alive() {
    ...
    let result = profile.evaluate("throw new Error('boom')").expect("protocol should not fail against a live worker");
    assert!(result.is_err(), "a throwing script should be reported as Err, not silently swallowed");

    // The worker must survive its own page's exception and keep answering.
    assert!(profile.ping().expect("protocol should not fail"), "worker should still respond after a script threw");

    // And a following EVAL still runs - one bad script doesn't poison the context.
    let result = profile.evaluate("40 + 2").expect("protocol should not fail");
...
fn a_pages_console_output_is_drainable_over_the_protocol() {
    ...
    let messages = profile.console().expect("protocol should not fail against a live worker");
...
fn an_uncaught_page_script_error_shows_up_in_the_console_stream() {
    ...
    let messages = profile.console().expect("protocol should not fail");
    assert_eq!(messages.len(), 1, "exactly one entry for the single uncaught error");
    assert_eq!(messages[0].0, "error");
    assert!(messages[0].1.starts_with("Uncaught "), "got: {}", messages[0].1);
...
fn console_is_empty_for_a_page_that_never_logs() {
    ...
    assert!(
        profile.console().expect("protocol should not fail").is_empty(),
