...
20. `[~]` Default actions — button/input submit/reset done (2026-09-12): an unprevented click on `<button>` (default type, or explicit `submit`) or `<input type="submit">` triggers its nearest ancestor `<form>`'s real `requestSubmit()`; `type="reset"` triggers `reset()` (`js_runtime::dom_bindings::forms::run_default_click_action`, hooked into both real tree-walking dispatch paths, `events::dispatch`/`dispatch_existing`). Still `[ ]`: `<a href>` click navigation (needs a host round-trip signal that doesn't exist yet), focus navigation, selection, scrolling defaults.
