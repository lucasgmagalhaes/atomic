...
| Downloads & history por profile | storage (fase 4, File/Blob) + net | **gap parcial** — storage tem cookies/localStorage/sessionStorage/IndexedDB reais e persistidos, incl. cookies agora trocados de fato via `net::get_with_headers` em toda navegação/fetch de `profile-worker` (`fetch_with_cookies`, request `Cookie` + `Set-Cookie` gravado no jar); `js-runtime` agora tem `Blob`/`File`/`URL.createObjectURL` reais (`crates/js-runtime/src/blob.rs` — bytes reais, `slice`/`text`/`arrayBuffer` genuínos via Promise), fechando o item File/Blob do roadmap da fase 4; `net` agora tem `download`/`download_with_headers` reais (`crates/net/src/lib.rs` — faz o GET e grava o corpo em disco via `std::fs::write`, testado contra HTTPS real); ainda falta UI/list de downloads e histórico de navegação — `download` é só a metade de rede, não gerencia uma lista/estado de downloads |
| Settings: Network (proxy mode, DNS, WebRTC leak guard) | net, security | **parcialmente coberto** — `net` tem tunelamento de proxy real (`ProxyConfig` + `get_via_proxy`, CONNECT real) e agora fiado ponta a ponta até `profile`/`profile-worker` (`Profile::spawn_with_proxy`, `parse_proxy_arg`, todo fetch do worker passa pelo proxy configurado), testado com servidores TCP locais reais incl. prova de que o CONNECT realmente aconteceu; falta só a UI (`apps/shell`) pra escolher/editar o proxy por perfil e "shared pool" (seleção entre vários proxies configurados); `net` agora também tem resolução DNS customizada real (`resolve_a`/`get_via_dns` em `crates/net/src/dns.rs` — cliente DNS-over-UDP escrito à mão, RFC 1035, testado incl. prova de que ignora o resolver do SO), fechando a parte "DNS" deste gap; agora fiado ponta a ponta até `profile`/`profile-worker` também (`Profile::spawn_with_dns`/`spawn_with_proxy_and_dns`, `parse_dns_arg`, todo fetch do worker usa o servidor DNS configurado quando não há proxy — proxy sempre tem prioridade se ambos setados), testado com servidor DNS-over-UDP local real incl. prova de que resolve um domínio inexistente só por causa do resolver customizado; "WebRTC leak guard" continua sem spec (nem WebRTC existe neste browser ainda) |
...
| Settings: Privacy (isolamento de sessão, clear on close, credential vault, telemetria) | storage, security | parcialmente coberto — `security` agora tem `CredentialVault` real (`crates/security/src/vault.rs`: AES-256-GCM real via `encrypt`/`decrypt` já existentes, persistido em disco, testado incl. "chave errada falha"), fechando o "Credential vault · Encrypted" do mockup; a metade "OS keychain" segue não implementada (chave mestra fica num arquivo local, não no Windows Credential Manager/macOS Keychain/Linux Secret Service) — clear on close/telemetria continuam sem spec |
