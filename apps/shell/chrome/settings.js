...
// `Button`/`Row`/`mount` kit (`lib/ui.js`) — same reasoning `toolbar.js`
// documents: each button owns its own real `click` listener, so
// `ChromeEngine`'s `dispatch_click` (a real `dispatchEvent("click")` on the
// resolved element) reaches it directly, no delegated root listener
