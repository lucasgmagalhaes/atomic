...
fn cursor_starts_before_the_first_entry() {
    ...
    let cursor = db.store("s").unwrap().open_cursor(&KeyRange::all(), CursorDirection::Next);
    assert_eq!(cursor.key(), None, "key() before any advance() should be None");
}

#[test]
fn cursor_visits_keys_in_ascending_order_by_default() {
    let db = seeded_store("ascending");
    let mut cursor = db.store("s").unwrap().open_cursor(&KeyRange::all(), CursorDirection::Next);
...
fn cursor_visits_keys_in_descending_order_when_asked() {
    ...
    let mut cursor = db.store("s").unwrap().open_cursor(&KeyRange::all(), CursorDirection::Prev);
...
fn key_range_only_visits_the_matching_key() {
    ...
    let mut cursor = db.store("s").unwrap().open_cursor(&KeyRange::only("b"), CursorDirection::Next);
    assert!(cursor.advance());
...
fn key_range_lower_bound_excludes_keys_below_it() {
    ...
    let mut cursor = db.store("s").unwrap().open_cursor(&KeyRange::lower_bound("b", false), CursorDirection::Next);
    let mut seen = Vec::new();
    while cursor.advance() {
        seen.push(cursor.key().unwrap().to_string());
    }
...
fn key_range_lower_bound_open_excludes_the_bound_itself() {
    ...
    let mut cursor = db.store("s").unwrap().open_cursor(&KeyRange::lower_bound("b", true), CursorDirection::Next);
    let mut seen = Vec::new();
    while cursor.advance() {
        seen.push(cursor.key().unwrap().to_string());
    }
...
fn key_range_bound_restricts_to_a_closed_interval() {
    ...
    let mut cursor = db.store("s").unwrap().open_cursor(&KeyRange::bound("b", "c", false, false), CursorDirection::Next);
    let mut seen = Vec::new();
    while cursor.advance() {
        seen.push(cursor.key().unwrap().to_string());
    }
