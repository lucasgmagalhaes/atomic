...
fn jar_matches_by_domain_path_and_scheme() {
  ...
  jar
    .set_from_header("a=1; Domain=example.com; Path=/app", "example.com")
    .unwrap();
...
fn expired_cookies_are_persisted_but_never_matched() {
  ...
  let mut jar = CookieJar::open(&path).unwrap();
  jar
    .set_from_header("a=1; Max-Age=0", "example.com")
    .unwrap();
...
fn matching_with_context_excludes_strict_and_lax_cookies_from_a_cross_site_request() {
  ...
  let mut jar = CookieJar::open(&path).unwrap();
  jar
    .set_from_header("a=1; SameSite=Strict", "example.com")
    .unwrap();
  jar
    .set_from_header("b=2; SameSite=Lax", "example.com")
    .unwrap();
...
fn matching_with_context_still_sends_same_site_none_cross_site() {
  ...
  let mut jar = CookieJar::open(&path).unwrap();
  jar
    .set_from_header("a=1; SameSite=None; Secure", "example.com")
    .unwrap();
