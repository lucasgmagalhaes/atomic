...
use atomic::automation_bridge::run_script;
...
fn a_plain_script_with_no_pane_call_still_evaluates() {
    ...
    let result = run_script(&workspace, [(PANE_ID, &browser)], "1 + 1");
...
fn a_script_can_address_two_different_live_panes() {
    ...
    let browser_a = BrowserView::spawn(200, 150);
    let browser_b = BrowserView::spawn(200, 150);
