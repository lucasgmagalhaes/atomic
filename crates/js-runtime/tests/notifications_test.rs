...
fn permission_starts_as_default() {
    ...
    assert_eq!(
        ctx.eval("Notification.permission", "<test>").unwrap(),
        "default"
    );
}
...
fn permission_becomes_granted_only_after_request_permission_resolves() {
    ...
    ctx.eval(
        "globalThis.seen = null; Notification.requestPermission().then((p) => { seen = p; });",
        "<test>",
    )
    .unwrap();

    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(2);
    while ctx.eval("seen === null", "<test>").unwrap() == "true"
...
fn constructor_reads_back_real_title_and_options() {
    ...
    assert_eq!(
        result,
        "Claim ready|Your idle claim is ready|idle-claim|/icon.png"
    );
}
...
fn constructor_without_options_leaves_extras_empty() {
    ...
    let ctx = Context::new(&rt);
    let result = ctx
        .eval("`${new Notification('just a title').body}`", "<test>")
        .unwrap();
    assert_eq!(result, "");
}
...
fn close_does_not_throw_and_is_idempotent() {
    ...
    let result = ctx
        .eval(
            "const n = new Notification('x'); n.close(); n.close(); true",
            "<test>",
        )
        .unwrap();
...
fn permission_is_isolated_per_context() {
    ...
    ctx1.eval("Notification.requestPermission()", "<test>")
        .unwrap();
    ctx1.run_pending_timers();

    assert_eq!(
        ctx1.eval("Notification.permission", "<test>").unwrap(),
        "granted"
    );
    assert_eq!(
        ctx2.eval("Notification.permission", "<test>").unwrap(),
        "default"
    );
}
