use atomic::browser_view::BrowserView;
use atomic::downloads::Downloads;
use atomic::history::History;
use atomic::resource_monitor::PaneMonitor;
use atomic::workspace::WorkspaceManager;
...
/// `next_pane_id`; the "+ Add Profile" modal (`AtomicApp::create_profile`)
...
pub struct Pane {
    ...
    /// `Vec` AtomicApp would have to keep in sync with `panes`) so it
    ...
    /// land under `%TEMP%/atomic-downloads/<pane-id>/`.
...
/// field - see `AtomicApp::create_profile`) instead of the two-step
...
/// `AtomicApp::next_pane_id`) - since [`BrowserView::spawn_with_identity`]
...
pub fn spawn_pane_with_proxy(
    ...
    let downloads_dir = std::env::temp_dir().join("atomic-downloads").join(&id);
    let history_path = std::env::temp_dir()
        .join("atomic-shell-history")
        .join(format!("{id}.txt"));
