...
fn temp_vault_path(label: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("nimble-vault-keychain-test-{label}-{}.bin", std::process::id()))
}
...
fn set_then_reopen_reads_back_the_same_entry() {
    ...
    {
        let mut vault = CredentialVault::open_or_create_with_keychain(&path).expect("first open should provision a fresh keychain key");
        vault.set("login.email", "user@example.com").expect("set should succeed");
    }

    let vault = CredentialVault::open_or_create_with_keychain(&path).expect("second open should reuse the same keychain key");
...
fn second_open_reuses_the_same_keychain_key_not_a_freshly_generated_one() {
    ...
    {
        let _vault = CredentialVault::open_or_create_with_keychain(&path).expect("first open should provision a key");
    }
    let key_after_first_open = keychain::read_credential(&target).expect("a key should now exist in the credential store");

    {
        let _vault = CredentialVault::open_or_create_with_keychain(&path).expect("second open should succeed");
    }
    let key_after_second_open = keychain::read_credential(&target).expect("the key should still be there");

    assert_eq!(key_after_first_open, key_after_second_open, "opening twice must not rotate the key");
...
fn vault_file_cannot_be_decrypted_without_the_matching_keychain_key() {
    ...
    {
        let mut vault = CredentialVault::open_or_create_with_keychain(&path).expect("open should succeed");
    ...
    assert!(result.is_err(), "reopening with a freshly generated key should fail to decrypt the old vault file");
...
fn distinct_vault_paths_get_distinct_keychain_entries() {
    ...
    {
        let mut vault_a = CredentialVault::open_or_create_with_keychain(&path_a).expect("open a should succeed");
        vault_a.set("k", "value-a").expect("set should succeed");
    }
    {
        let mut vault_b = CredentialVault::open_or_create_with_keychain(&path_b).expect("open b should succeed");
        vault_b.set("k", "value-b").expect("set should succeed");
    }

    let vault_a = CredentialVault::open_or_create_with_keychain(&path_a).expect("reopen a should succeed");
    let vault_b = CredentialVault::open_or_create_with_keychain(&path_b).expect("reopen b should succeed");
