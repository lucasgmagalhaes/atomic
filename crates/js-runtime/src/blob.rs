...
static BLOB_CLASS_ID: AtomicU32 = AtomicU32::new(0);
...
unsafe fn blob_opaque(this_val: sys::JSValue) -> *mut BlobInner {
    let class_id = BLOB_CLASS_ID.load(Ordering::Relaxed);
    sys::JS_GetOpaque(this_val, class_id) as *mut BlobInner
}

unsafe extern "C" fn blob_finalizer(_rt: *mut sys::JSRuntime, val: sys::JSValue) {
    let ptr = blob_opaque(val);
    if !ptr.is_null() {
...
unsafe fn make_blob_object(ctx: *mut sys::JSContext, new_target: sys::JSValue, bytes: Vec<u8>, mime: String) -> sys::JSValue {
    let class_id = BLOB_CLASS_ID.load(Ordering::Relaxed);
...
unsafe extern "C" fn blob_constructor(
    ...
    make_blob_object(ctx, new_target, bytes, mime)
}
...
unsafe extern "C" fn file_constructor(
    ...
    let obj = make_blob_object(ctx, new_target, bytes, mime);
    if !sys::js_is_exception(&obj) {
...
unsafe extern "C" fn blob_slice(ctx: *mut sys::JSContext, this_val: sys::JSValue, argc: c_int, argv: *mut sys::JSValue) -> sys::JSValue {
    ...
    make_blob_object(ctx, sys::js_undefined(), sliced, mime)
}
...
unsafe fn ensure_blob_class(ctx: *mut sys::JSContext, name: &str, ctor: sys::JSCFunction) -> sys::JSClassID {
    ...
    // `Blob` and `File` share one class/opaque layout (`BlobInner`).
    // `JS_NewClassID` reuses the id already stashed in the static (via its
    // in/out pointer param) rather than allocating a new one, but
    // `JS_NewClass` must still run per-`JSRuntime` - each test/host gets
    // its own `Runtime`, and a class id from an earlier runtime is not
    // registered in a later one's class table (ignoring failure here,
    // same convention as `dom_bindings::ensure_node_class`: it just means
    // this runtime already has it, e.g. `File` after `Blob`).
    let class_id = sys::JS_NewClassID(rt, BLOB_CLASS_ID.as_ptr());
