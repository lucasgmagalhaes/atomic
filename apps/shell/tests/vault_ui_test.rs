...
fn open_with_keychain_uses_a_distinct_file_from_the_plain_vault() {
  ...
  let mut keychain_vault =
    vault_ui::open_with_keychain(&dir).expect("real Windows Credential Manager should back this");
  ...
  let reopened =
    vault_ui::open_with_keychain(&dir).expect("reopen should reuse the real keychain-stored key");
  assert_eq!(
