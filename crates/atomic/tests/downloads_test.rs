...
fn download_writes_a_real_file_and_records_a_success() {
    ...
    let bytes = record
        .result
        .as_ref()
        .expect("a real fetch of example.com should succeed");
    assert!(*bytes > 0);
...
fn a_failed_download_still_records_an_entry_with_the_error() {
    ...
    assert!(
        record.result.is_err(),
        "a bad URL should record Err, not silently succeed"
    );
    assert_eq!(
        downloads.entries().count(),
        1,
        "a failure should still produce a record"
    );
...
fn a_reopen_of_the_same_dir_sees_prior_real_download_records() {
    ...
    1,
    "a fresh Downloads over the same dir should load the real manifest left by the previous one"
  );
    ...
    assert!(
        record
            .result
            .as_ref()
            .expect("the persisted record should still show success")
            .clone()
            > 0
    );
